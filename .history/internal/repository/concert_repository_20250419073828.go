...
func (r *ConcertRepo) LockConcertByID(ctx context.Context, tx *sql.Tx, id int) (*model.Concert, error) {
	var c model.Concert
	err := tx.QueryRowContext(ctx, `SELECT id, name, available_tickets, start_time, end_time FROM concerts WHERE id = $1 AND start_time <= NOW() AND end_time >= NOW() FOR UPDATE`, id).
