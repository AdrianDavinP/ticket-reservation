...
func (h *ConcertHandler) BookTicket(ctx context.Context, req *pb.BookRequest) (*pb.BookResponse, error) {
	status, err := h.Service.BookTicket(ctx, req)
	if err != nil {
		return &pb.BookResponse{Status: status.Status, Message: err.Error()}, nil
	}
	return &pb.BookResponse{Status: status.Status, Message: "Ticket booked successfully"}, nil
}
