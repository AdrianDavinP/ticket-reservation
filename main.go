...
func main() {
	...
	maxCon, err := strconv.Atoi(os.Getenv("DB_MAX_CON"))
	if err != nil {
	...
	if err != nil {
		log.Fatalf("Error env DB_MAX_IDLE : %v", err)
	}

	conLifeTime, err := strconv.Atoi(os.Getenv("DB_CON_LIFETIME"))
	if err != nil {
		log.Fatalf("Error env DB_CONN_LIFETIME : %v", err)
	}

	db.SetMaxOpenConns(maxCon)
	db.SetMaxIdleConns(maxIdle)
	db.SetConnMaxLifetime(time.Duration(conLifeTime) * time.Minute)
